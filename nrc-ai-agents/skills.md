...
# Hallmark (installed 2026-07-19, not yet curated)

Installed via `npx skills add nutlope/hallmark` at explicit request — a **different install mechanism** from every other skill above. Those were fetched and copied by hand into `.claude/skills/<name>/`; this one was installed by the `skills` CLI (`skills@1.5.19`, Vercel Labs, MIT license — https://github.com/vercel-labs/skills), which stores the actual skill content at `.agents/skills/hallmark/` (a cross-agent-tool location shared by Antigravity, Codex, Gemini CLI, GitHub Copilot, and others) and symlinks `.claude/skills/hallmark → ../../.agents/skills/hallmark` for Claude Code, tracked in a new `skills-lock.json` at the project root. Like the UI/UX Pro Max bundle, **this has not been through a scoping/approval review** — treat the restrictions below as provisional.

- **Actual skill name (frontmatter):** `hallmark`
- **Folder location:** [`.agents/skills/hallmark/SKILL.md`](.agents/skills/hallmark/SKILL.md) (symlinked at `.claude/skills/hallmark/`)
- **Purpose:** An "anti-AI-slop" design skill with four verbs — default **build** (greenfield pages), `hallmark audit` (score an existing page against a 58-gate anti-pattern checklist, read-only), `hallmark redesign` (restructure an existing page's visual/interaction layer inside its current implementation boundaries), and `hallmark study` (extract structural "DNA" — macrostructure, type pairing, colour anchor — from a screenshot or URL, never pixel-cloning). It picks from 21 named page macrostructures, 14 nav archetypes, 8 footer archetypes, and either a 20-theme catalog or a bespoke OKLCH-palette-plus-font-pairing "custom" route, with a diversification rule so consecutive builds don't repeat the same structure/theme, and writes its own `tokens.css`, `.hallmark/log.json` (project memory for diversification), and optionally a portable `design.md` (only on explicit request).
- **When it should be used:** Only if explicitly asked to use Hallmark by name, or for a specific scoped ask matching one of its verbs (e.g., "audit this page against Hallmark's checklist"). Its own Step 0 pre-flight scan does read existing fonts/palette/framework signals before proposing anything and states what it will preserve vs. introduce — more self-aware about existing projects than most of the bundle skills above — but it still defaults to *introducing* a macrostructure, nav/footer archetype, and slop-test discipline of its own, which is exactly the kind of second, competing design authority this project has repeatedly guarded against.
- **When it should not be used:** Never as an automatic or default design authority for NRC — `redesign-existing-projects` remains primary, per the project-wide rule against combining multiple competing aesthetic directions automatically. Do not let its catalog theme rotation, OKLCH palette construction, or named macrostructures silently override NRC's established champagne/ivory/sage/bronze/charcoal/dark palette, Cormorant Garamond/Inter typography, or vanilla HTML/CSS/JS + Vite + Three.js stack. Do not let it write a `tokens.css` / `design.md` that fragments NRC's existing single-file `:root` custom-property system without explicit approval. Its `audit` verb (read-only, no edits) is the lowest-risk entry point if this skill is ever exercised on NRC.
- **Source repository:** https://github.com/nutlope/hallmark (installed via `nutlope/hallmark` through the `skills` CLI)
- **Important usage restrictions:** Treat identically to the UI/UX Pro Max bundle's restrictions — narrow, explicit-request-only use; never primary; never a silent palette/stack override. Its Tier C "generated illustration" enrichment path references third-party paid image APIs (Nanobanana/Gemini 2.5 Flash Image, Recraft, Midjourney, Flux) as options of last resort — none are configured in this project, and per the skill's own hierarchy most builds shouldn't need them (typography-only or hand-built CSS/SVG art is preferred first).
- **Supporting files:** `references/` (macrostructures, component/nav/footer cookbook, per-genre and per-theme specs, typography/color/layout/motion/copy rules, the 58-gate slop test, verb-specific guides for `audit`/`redesign`/`study`, custom-theme and design.md protocols).
